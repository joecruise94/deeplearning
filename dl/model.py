...
class CaptionGenerator(object):
    ...
    def myattention_layer(self, features, h, reuse=False):
        with tf.variable_scope('myattention_layer', reuse=reuse):
            ...
            alpha2 = tf.nn.softmax(alpha2)  ### N* 49
    ...
    def sca_layer(self, features, h, reuse=False):
        with tf.variable_scope('sca_layer', reuse=reuse):
            features_mean = tf.reshape(tf.reduce_mean(features,1, name='featuremean'),[-1,self.D,1])
            w1_1 = tf.get_variable('w1_1', [1, self.M], initializer=self.weight_initializer)
            b1_1 = tf.get_variable('b1_1', [self.M], initializer=self.const_initializer)
            pctx_1 = tf.tensordot(features_mean, w1_1,[[2],[0]])+b1_1
            ...
            alpha1 = tf.reshape(tf.tensordot(pctx1,w1_3,[[2],[0]])+b1_2, [-1, self.D])
            alpha1 = tf.nn.softmax(alpha1)  ## N * 512
            weighted_features = tf.multiply(features, tf.expand_dims(alpha1,1))
            w2_1 = tf.get_variable('w2_1', [self.D, self.M], initializer=self.weight_initializer)
            b2_1 = tf.get_variable('b2_1', [self.M], initializer=self.const_initializer)
            pctx_2 = tf.tensordot(weighted_features, w2_1,[[2],[0]])+b2_1
            #w2_2 = tf.get_variable('w2_2', [self.H, self.M], initializer=self.weight_initializer)
            pstate_2 = tf.expand_dims(tf.matmul(h, w1_2),1)
            pctx2 = tf.nn.tanh(pctx_2+pstate_2)
            w2_3 = tf.get_variable('w2_3', [self.M, 1], initializer=self.weight_initializer)
            b2_2 = tf.get_variable('b2_2', [1], initializer=self.const_initializer)
            alpha2 = tf.reshape(tf.tensordot(pctx2,w2_3,[[2],[0]])+b2_2, [-1, 49])
            alpha2 = tf.nn.softmax(alpha2)  ### N* 49
            alpha1_reshape = tf.expand_dims(alpha1,1)
            alpha2_reshape = tf.expand_dims(alpha2,2)
            context = tf.multiply(tf.multiply(features,alpha1_reshape),alpha2_reshape)
            context2 = tf.reshape(tf.reduce_sum(context,1),[-1,self.D], name='context')  # 512
            #contextnorm = tf.sqrt(tf.reshape(tf.reduce_sum(tf.square(context2),1),[-1,1]))
            #contextnorm = contextnorm+ tf.constant([1e-8])
            #context_normal = tf.divide(context2,contextnorm)
            context3 = tf.reshape(tf.reduce_sum(context,2),[-1,49], name='context3') #49
            #contextnorm2 = tf.sqrt(tf.reshape(tf.reduce_sum(tf.square(context3),1),[-1,1]))
            #contextnorm2 = contextnorm2+ tf.constant([1e-8])
            #context_normal2 = tf.divide(context3,contextnorm2)       
            #return context_normal,context_normal2, alpha2, alpha1
            return context2,context3, alpha2, alpha1, w1_2
    ...
    def mydecode_lstm(self, x, h, context,context2, dropout=False, reuse=False):
        with tf.variable_scope('logits', reuse=reuse):
            w_h = tf.get_variable('w_h', [self.H, self.M], initializer=self.weight_initializer)
            b_h = tf.get_variable('b_h', [self.M], initializer=self.const_initializer)
            w_out = tf.get_variable('w_out', [self.M*2, self.V], initializer=self.weight_initializer)
            b_out = tf.get_variable('b_out', [self.V], initializer=self.const_initializer)
            ...
            h_logits = tf.concat([h_logits,contextvec],1)
            #h_logits = tf.nn.tanh(h_logits)
            #if dropout:
            #    h_logits = tf.nn.dropout(h_logits, 0.5)
            out_logits = tf.matmul(h_logits, w_out) + b_out
    ...
    def mydecode_lstm2(self, sentinal, h, context,context2,alpha,alpha2, wh, dropout=False, reuse=False):
        with tf.variable_scope('mylogits', reuse=reuse):

            ws = tf.get_variable('ws', [self.H, self.D], initializer=self.weight_initializer)
            bs = tf.get_variable('bs', [self.D], initializer=self.const_initializer)
            sentinal_vec = tf.nn.tanh(tf.matmul(sentinal,ws)+bs + tf.matmul(h,wh))
            wss = tf.get_variable('wss', [self.D, 1], initializer=self.weight_initializer)
            sentinal_z = tf.matmul(sentinal_vec,wss)
            zt_cat = tf.concat([alpha,sentinal_z],1)
            zt_cat_softmax = tf.nn.softmax(zt_cat)
            betat = zt_cat_softmax[:,-1:]


            ctx1w = tf.get_variable('ctx1w', [self.D, self.M], initializer=self.weight_initializer)
            ctx1b = tf.get_variable('ctx1b', [self.M], initializer=self.const_initializer)
            ctx2w = tf.get_variable('ctx2w', [49, self.M], initializer=self.weight_initializer)
            ctx2b = tf.get_variable('ctx2b', [self.M], initializer=self.const_initializer)
            w_h = tf.get_variable('w_h', [self.H, self.M], initializer=self.weight_initializer)
            b_h = tf.get_variable('b_h', [self.M], initializer=self.const_initializer)
            w_out = tf.get_variable('w_out', [self.M, self.V], initializer=self.weight_initializer)
            b_out = tf.get_variable('b_out', [self.V], initializer=self.const_initializer)
            ...
            ctx1logit = tf.nn.dropout(tf.matmul(tf.nn.dropout(context,0.5),ctx1w)+ctx1b,0.5)
            ctx2logit = tf.nn.dropout(tf.matmul(tf.nn.dropout(context2,0.5),ctx2w)+ctx2b,0.5)
            h_logits +=(ctx2logit+ctx1logit)*(1-betat)+betat*sentinal_vec
            if dropout:
                h_logits = tf.nn.dropout(h_logits, 0.5)
            out_logits = tf.matmul(h_logits, w_out) + b_out
            return out_logits
    ...
    def _pooling(self, features):
        with tf.variable_scope('pooling_features'):
            inputs = tf.reshape(features,[-1,14,14,self.D])
            pool1 = tf.layers.max_pooling2d(inputs=inputs, pool_size=[2,2], strides=2)
            outputs = tf.reshape(pool1,[-1,49,self.D])
            return outputs 

    def _featuresemantic(self, poolfeatures, prevh, x, reuse):
        with tf.variable_scope('featuresemantic', reuse=reuse):
            w = tf.get_variable('w', [self.D, self.D], initializer=self.weight_initializer)
            b = tf.get_variable('b', [self.D], initializer=self.const_initializer)
            meanfeature = tf.reshape(tf.reduce_mean(poolfeatures, axis=1),[-1,self.D])
            fsemantic = tf.matmul(meanfeature,w)+b
            # hw = tf.get_variable('hw', [self.H, 1], initializer=self.weight_initializer)
            # xw = tf.get_variable('xw', [self.M, 1], initializer=self.weight_initializer)
            # gate = tf.nn.sigmoid(tf.matmul(prevh, tf.nn.dropout(hw,0.5))+tf.matmul(x,tf.nn.dropout(xw,0.5)))
            # out = tf.multiply(fsemantic,gate)
            out = tf.nn.relu(fsemantic)
            return out

    def _sentinal(self, x, prevh, c, reuse):
        with tf.variable_scope('sential', reuse=reuse):
            wx = tf.get_variable('wx', [self.M, self.H], initializer=self.weight_initializer)
            bx = tf.get_variable('bx', [self.H], initializer=self.const_initializer)
            wh = tf.get_variable('wh', [self.H, self.H], initializer=self.weight_initializer)
            bh = tf.get_variable('bh', [self.H], initializer=self.const_initializer)
            gt = tf.nn.sigmoid(tf.matmul(x,wx)+bx+tf.matmul(prevh,wh)+bh)
            out = tf.multiply(gt,c)
            return out
    ...
    def build_model(self):
        ...
        features_pool = self._pooling(features)
        ...
        for t in range(self.T):
            ...
            precontext = self._featuresemantic(features_pool, h, x[:,t,:], reuse=(t!=0))  # N * 512
            prevh = h
            with tf.variable_scope('lstm', reuse=(t!=0)):
                _, (c, h) = lstm_cell(inputs=tf.concat([x[:,t,:],precontext],1), state=[c, h])
                #_, (c, h) = lstm_cell(inputs=x[:,t,:], state=[c, h])
            sentinal = self._sentinal(x[:,t,:], prevh, c, reuse=(t!=0))
            context, context2, alpha, alpha2,wh = self.sca_layer(features_pool, c, reuse=(t!=0))
            # N * 512 
            if self.selector:
                context, beta = self._selector(context, h, reuse=(t!=0))
            alpha_list.append(alpha)
            alpha2_list.append(alpha2)
            logits = self.mydecode_lstm2(sentinal, h, context,context2,alpha, alpha2, wh, dropout=self.dropout, reuse=(t!=0))
	...
	alpha2_list = []
        ...
        for t in range(max_len):
            ...
            precontext = self._featuresemantic(features_pool, h, x, reuse=(t!=0))  # N * 512
            prevh = h
            #precontext, prealpha = self._attention_layer(features,features_proj, h, reuse=(t!=0))
            ...
            #with tf.variable_scope('lstm', reuse=(t!=0)):
            #    _, (c, h) = lstm_cell(inputs=x, state=[c, h])
            sentinal = self._sentinal(x, prevh, c, reuse=(t!=0))
            ...
            alpha_list.append(alpha)
	    alpha2_list.append(alpha2)
            logits = self.mydecode_lstm2(sentinal, h, context,context2,alpha,alpha2, wh, reuse=(t!=0))
